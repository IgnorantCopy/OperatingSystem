...
void syscallOpen(struct StackFrame *sf) {
	...
	if (ret == 0) { // file exist
		...
		int16_t destType = destInode.type;
		if ((flags >> 3) % 2 == 1 && destType != DIRECTORY_TYPE) {	// O_DIRECTORY set
			pcb[current].regs.eax = -1;
			return;
		}
		if (destType == UNKNOWN_TYPE || destType == FIFO_TYPE || destType == SOCKET_TYPE) {
			pcb[current].regs.eax = -1;
			return;
		}
		...
		allocInode(&sBlock, gDesc, &fatherInode, fatherInodeOffset, &destInode, &destInodeOffset, str + size + 1, type);
...
void syscallWrite(struct StackFrame *sf) {
	...
	const int fd = (int)sf->ecx - MAX_DEV_NUM;
	if (fd >= 0 && fd < MAX_FILE_NUM && file[fd].state == 1) {
		syscallWriteFile(sf);
	}
	return;
}
...
void syscallWriteFile(struct StackFrame *sf) {
	...
	for (i = 0, j = remainder; i < size;) {
		...
		for (; j < SECTOR_SIZE * SECTORS_PER_BLOCK && i < size; i++, j++) {
			buffer[j] = str[i];
...
void syscallLseek(struct StackFrame *sf) {
	int fd = (int)sf->ecx; // file descriptor
	...
	switch(sf->ebx) { // whence
		...
		case SEEK_END:
			// TODO: if SEEK_END
			if (offset <= 0) {
				file[fd - MAX_DEV_NUM].offset = inode.size + offset;
			} else {
				pcb[current].regs.eax = -1;
				return;
			}
...
void syscallClose(struct StackFrame *sf) {
	...
	file[fd - MAX_DEV_NUM].inodeOffset = 0;
	file[fd - MAX_DEV_NUM].offset = 0;
	file[fd - MAX_DEV_NUM].flags = 0;
	pcb[current].regs.eax = 0;
	return;
}
