...
struct GateDescriptor {
	uint32_t offset_15_0      : 16; // offset bits 0..15
	uint32_t segment          : 16; // segment selector
	uint32_t pad0             : 8;  // unused, for alignment
	uint32_t type             : 4;  // gate type (STS_TG32 for trap gate and STS_IG32 for interrupt gate)
	uint32_t system           : 1;  // 0 = task gate, 1 = system gate
	uint32_t privilege_level  : 2;  // Descriptor Privilege Level
	uint32_t present          : 1;  // 1 for present and 0 for absent
	uint32_t offset_31_16     : 16; // offset bits 16..31
};
